...
void CardNine::ReadCardParameters(Grid* pGrid) {
	if (number == 0) {
		...
		pOut->PrintMessage("CardNine: Please enter cardPrice...");
		cardPrice = pIn->GetInteger(pOut);

		pOut->PrintMessage("CardNine: Please enter fees...");
...
void CardNine::Apply(Grid* pGrid, Player* pPlayer) {
	...
	Card* Card9Apply;
	Card9Apply->SetCardNumber(9);
	Card9Apply->Apply(pGrid, pPlayer);
	...
	if (isBought == 0) {
		if (pPlayer->GetWallet() >= cardPrice) {
			pOut->PrintMessage("Do you want to buy cardNine? y/n");
