...
int Input::GetInteger(Output *pO) const 
{
	...
	int val = stoi(GetSrting(pO));		// val is the integer to be returned

	return val; // this line should be changed with your implementation
}
...
CellPosition Input::GetCellClicked() const
{
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		if ( y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			...
			int Vertcell = y / UI.CellHeight - 1;
			int Horizcell = x / UI.CellWidth;


			cellPos.SetHCell(Horizcell);
			cellPos.SetVCell(Vertcell);
		}
	}
